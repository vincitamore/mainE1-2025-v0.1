  ...
  async analyzeRequest(
    ...
    const response = await this.llmProvider.generate(
      [
        {
          role: 'system',
          content: 'You are the Orchestrator - a high-level planning agent that analyzes user requests and plans multi-file operations. Always return valid JSON.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const parsed = safeJSONParse<any>(jsonStr, null, 'orchestrator');
...
Always return valid JSON without markdown formatting.`
    ...
    const jsonStr = extractJSON(response.content);
    const parsed = safeJSONParse<any>(jsonStr, null, 'orchestrator-planning');
